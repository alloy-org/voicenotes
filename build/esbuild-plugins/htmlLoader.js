...
export const HtmlLoaderPlugin = {
    ...
    setup(build) {
        build.onResolve({ filter: /inline:.*\.html$/ }, args => {
            try {
                const absolutePath = path.resolve(args.resolveDir, args.path.substring(7));
                // Convert to relative path from the plugin directory for the namespace
                const pluginDir = process.argv[2]; // This is the plugin directory path passed as argument
                const relativePath = path.relative(pluginDir, absolutePath);
                return {
                    path: relativePath,
        ...
        build.onLoad({ filter: /.*/, namespace: 'inline-html' }, async (args) => {
            // The path is now the relative path, resolve it to get the actual path
            const pluginDir = process.argv[2];
            const actualPath = path.resolve(pluginDir, args.path);
            
            let html = await fs.readFile(actualPath, 'utf8');
            let watchFiles = [];
            let errors = [];
            [html, watchFiles, errors] = await inlineAssetsOfHTML(html, actualPath, 'js');
            let watchFiles2 = [];
            let errors2 = [];
            [html, watchFiles2, errors2] = await inlineAssetsOfHTML(html, actualPath, 'css');
...
export async function inlineAssetsOfHTML(html, htmlPath, assetType) {
    ...
    for (const match of matches) {
        ...
        if (assetPath.startsWith('http://') || assetPath.startsWith('https://')) {
            console.log(`Skipping external ${assetType}: ${assetPath}`);
            continue;
        }
